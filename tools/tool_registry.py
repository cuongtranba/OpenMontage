...
class ToolRegistry:
    ...
    def _load_dotenv() -> None:
        """Load .env then .env.local into os.environ so tools can find API keys.

        `.env` provides shared defaults (only set when absent); `.env.local` is
        a gitignored local override whose values always win.
        """
