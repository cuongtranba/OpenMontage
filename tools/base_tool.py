...
def _load_dotenv() -> None:
    """Load .env then .env.local into os.environ once at import time.
    ...
    `.env` provides shared defaults (only set when a variable is absent);
    `.env.local` is a gitignored local override whose values always win.
    """
    import re
    root = Path(__file__).resolve().parent.parent
    for env_path, override in ((root / ".env", False), (root / ".env.local", True)):
